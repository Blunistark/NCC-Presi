...
const Camps = () => {
    ...
    return (
        <PageContainer title="Camps" description="NCC Camps">
            {/* Navigation Header */}
            <Stack direction="row" justifyContent="space-between" alignItems="center" mb={3}>
                <Breadcrumbs aria-label="breadcrumb">
                    ...
                    <Typography color="text.primary">Camps</Typography>
                </Breadcrumbs>
            </Stack>

            <Box>
                {loading && <CircularProgress />}
                {!loading && events.length === 0 && <Alert severity="info">No Camps found.</Alert>}
                        ...
                        <Grid size={{ xs: 12, sm: 6, md: 4 }} key={event["Event ID"]}>
                            <Box component={Link} href={`/parades/camps/${event["Event ID"]}`} sx={{ textDecoration: 'none' }}>
                                <BlankCard>
                                    <CardContent sx={{ p: 0 }}>
                                        {/* Header Color Strip */}
                                        <Box sx={{ height: '8px', bgcolor: '#FFAE1F', width: '100%' }} />
